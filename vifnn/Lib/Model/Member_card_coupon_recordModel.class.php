	...
	public function get_coupon($wechat_id, $token, $price, $cid = 0)
	{
		...
		if ($result) {
			...
			foreach ($result as $row) {
				...
				if($cid>0){
				  	if(($row['company_id']==$cid) || !($row['company_id']>0)){
				       $list[$row['id']] = $row;
				    }
				}else{
				    $list[$row['id']] = $row;
