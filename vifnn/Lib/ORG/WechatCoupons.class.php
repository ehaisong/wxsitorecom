...
class WechatCoupons extends apiOauth
{
	...
	public function getLogoUrl($logourl,$id){
		...
		$tmplogourl = str_replace(array('http://',$_SERVER['HTTP_HOST']),'',$logourl);
		$tmplogourl = getcwd().'/'.ltrim($tmplogourl,'/');
		$logo 	= file_get_contents($tmplogourl);
		if (!$logo){
			$logo 	= $this->curlGet($logourl);//又拍云
		}
