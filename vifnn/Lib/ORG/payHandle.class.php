...
final class payHandle{
	...
	public function beforePay($id, $first = 0){
		...
		if (key_exists('third_id',$thisOrder)){
			return array("orderid" => $thisOrder["orderid"], "cid" => isset($thisOrder["cid"]) ? $thisOrder["cid"] : 0, "old_price" => $old_price, "price" => $price, "wecha_id" => $thisOrder["wecha_id"], "token" => $thisOrder["token"], "paid" => $thisOrder["paid"], "third_id" => $thisOrder["third_id"],'havepaid'=>$thisOrder['havepaid']); //自行增加 'havepaid'=>$thisOrder['havepaid']
		}else {
			return array("orderid" => $thisOrder["orderid"], "cid" => isset($thisOrder["cid"]) ? $thisOrder["cid"] : 0, "old_price" => $old_price, "price" => $price, "wecha_id" => $thisOrder["wecha_id"], "token" => $thisOrder["token"], "paid" => $thisOrder["paid"], "transactionid" => $thisOrder["transactionid"],'havepaid'=>$thisOrder['havepaid']); //自行增加 'havepaid'=>$thisOrder['havepaid']
	...
	public function afterPay($id, $third_id = "", $transaction_id = ""){
		...
		file_put_contents($_SERVER["DOCUMENT_ROOT"] . "/Datavifnn/conf/4" . $this->token . ".txt", json_encode($thisOrder));		
		...
		if (($this->payType != "daofu") && ($this->payType != "dianfu")) {
			...
			/***********这里对接收银台，请勿删除***********/
			if(file_exists($_SERVER['DOCUMENT_ROOT'].'/Cashier/vifnn/base.php')){
