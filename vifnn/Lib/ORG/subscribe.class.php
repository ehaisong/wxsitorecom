...
class subscribe
{
	...
	public function sub()
	{
		...
		if (!$uid) {
			$foid = D('Userfollow')->setsubscribe($this->token, $this->wecha_id, 1, 0);
			if (empty($foid)) {
				return false;
			}
		...
		$result = $this->advanceReply($datainfo['wechaname'], $access_token);
		return $result;
	}
							...
							foreach ($img_list as $k => $v) {
								if ($v['url'] != '') {
									if (strpos($v['url'], '{siteUrl}') !== false || strpos($v['url'], '{wechat_id}') !== false) {
										$url = str_replace(array('{siteUrl}', '{wechat_id}'), array($this->siteurl, $this->wecha_id), $v['url']);
							...
							if (!empty($img)) {
								if ($img['url'] != '') {
									if (strpos($img['url'], '{siteUrl}') !== false || strpos($img['url'], '{wechat_id}') !== false) {
										$url = str_replace(array('{siteUrl}', '{wechat_id}'), array($this->siteurl, $this->wecha_id), $img['url']);
