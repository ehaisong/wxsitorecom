...
class subscribe {
	...
	public function sub() {
		...
		if ($this->thisWxUser['appid'] && $this->thisWxUser['winxintype'] == 3) {
			...
			if ($classData->subscribe && $classData->subscribe == 1) {
				...
				//更新用户信息 @xulinyang
				$updateData['wechaname']=$datainfo['wechaname'];
				$updateData['sex']=$datainfo['sex'];
				$updateData['portrait']=$datainfo['portrait'];
				$updateData['city']=$datainfo['city'];
				$updateData['province']=$datainfo['province'];
			}
		...
		$uid=M('Userinfo')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id))->getField('id');
		if (!$uid) {
			//首次关注
			$foid = D('Userfollow')->setsubscribe($this->token, $this->wecha_id, 1, 0); //新增关注表
			if (empty($foid)) {
				return false;
			}
			//新增用户表
		...
		else
		{
			if(!empty($updateData))
			{
				M('Userinfo')->where(array('id'=>$uid))->save($updateData);
			}
		}
		M('Userinfo')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id))->setField('issub', '1');
		// 清空关注缓存
		//S('sub_' . $this->token . '_' . $this->wecha_id, null);
		D('Userfollow')->setsubscribe($this->token, $this->wecha_id, 1); //更新关注表
		//存入
	...
	*/
	...
	public function unsub() {
		D('Userfollow')->setsubscribe($this->token, $this->wecha_id, -1); //更新关注表为－1
		...
		if (!empty($reply_list)) {
			foreach ($reply_list as $key => $value) {
				...
				if ($vs_timestamp < $_SERVER['REQUEST_TIME'] && $_SERVER['REQUEST_TIME'] < $ve_timestamp) {
					...
					case 1:
						$img = M('img')->where(array('id' => $value['relevance_id']))->find(); //单图文
						if (!empty($img)) {
							$url = $this->siteurl . '/index.php?g=Wap&m=Index&a=Replycontent&id=' . $value['relevance_id'] . '&token=' . $this->token;
							$return = array(array(array($img['title'], $img['text'], $img['pic'], $url)), 'news');
						} else {
							$return = false;
						}
						break;
						...
						if (($value['isrepeat'] == 1 && (int) $times < $value['times']) || ($value['isrepeat'] == 2 && $times == '')) {
							...
							if (!empty($card)) {
								if ($times == '') {
									...
									$update = M('subscribe_times')->where(array('wecha_id' => $this->wecha_id, 'token' => $this->token, 'rid' => $value['id']))->save(array('times' => array('exp', 'times+1'), 'last_rtime' => time()));
									if ($update) {
										$return = $this->sendCard($card['card_id'], $access_token);
										if (!$return) {
											$return = false;
					...
					case 7: //外链图文或活动
						...
						if (!empty($img)) {
							if ($img['url'] != "") {
								...
								$return = array(array(array($img['title'], $img['text'], $img['pic'], $img['url'])), 'news');
							} else {
								$return = false;
							}
						} else {
	...
	private function send_hongbao($wechaname = '', $params = array()) {
		$wechaname = ($wechaname != "") ? $wechaname : '匿名用户';
		//订阅号不能发红包
		if ($this->wecha_id == "") {return false;}
		$config = array();
		...
		if ($res['status'] == 'SUCCESS') {
			M('directhongbao')->where(array('id' => $params['id']))->save(array('total_money' => array('exp', 'total_money-' . $config['money'])));
			$record = array();
