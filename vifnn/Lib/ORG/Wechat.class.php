...
class Wechat
{
	...
	private function image($imageData)
	{
		$imageData = str_replace(C('site_url'), '.', $imageData);
		$material = new WxMaterialModel();
		$result = $material->localToWeixin($this->token)->uploadImage($imageData);

		if ($result) {
			$this->data['Image'] = array('MediaId' => $result['media_id']);
		}
	}
}
