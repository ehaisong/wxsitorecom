    ...
    public function index($orderid) {
        ...
        if ($order) {
            ...
            if ($order['paid']=='1') {
				...
				$printtype=$order['advancepay'] > 0 ? "点餐预定金" : "餐饮付款";
                $op = new orderPrint();
                ...
                if ($kitchens_list = D('Dish_kitchen')->where(array('cid' => $cid))->select()) {
	                ...
	                foreach ($temp as $dish) {
	                	if (isset($dish['j_c']) && !empty($dish['j_c'])){
	                ...
	                unset($print_msg['companyname'], $print_msg['companytel'], $print_msg['price'], $print_msg['total']);
					$print_msg['list']=array();
	                $t_list = !empty($j_list) ? $j_list : $t_list;
