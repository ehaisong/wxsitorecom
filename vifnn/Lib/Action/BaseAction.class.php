...
class BaseAction extends Action{
	...
	protected function _initialize(){
		...
		if (!$this->isAgent){
			$this->agentid=0;
			if (!C('site_logo')){
				$f_logo='tpl/Home/vifnn/common/images/logo-vifnn.png';
			}else {
				$f_logo=C('site_logo');
			}
			...
			$f_qq=C('site_qq');
			$f_ipc=C('ipc');
			$f_qrcode='tpl/Home/vifnn/common/images/ewm2.jpg';
	...
	protected function all_insert($name='',$back='/index'){
		...
		$db=D($name);
		$postData = $_POST;
		$postData["token"] = session("token");
		if ($db->create($postData) === false) {
