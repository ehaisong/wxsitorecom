...
class AgentAction extends BackAction
{
	...
	public function index()
	{
		...
		if (!$nodeExist) {
			$submenu = array('name' => 'AgentPrice', 'title' => '优惠套餐包', 'status' => 1, 'remark' => '0', 'pid' => $firstNode['id'], 'level' => 2, 'sort' => 0, 'display' => 2);
			$submenuRowID = M('Node')->add($submenu);
			$row2 = array('name' => 'add', 'title' => '添加', 'status' => 1, 'remark' => '0', 'pid' => $submenuRowID, 'level' => 3, 'sort' => 0, 'display' => 2);
			M('Node')->add($row2);
		}
		...
		if (!$buyRecordNode) {
			$submenu = array('name' => 'AgentBuyRecords', 'title' => '消费记录', 'status' => 1, 'remark' => '0', 'pid' => $firstNode['id'], 'level' => 2, 'sort' => 0, 'display' => 2);
			$submenuRowID = M('Node')->add($submenu);
		}
	...
	public function add()
	{
		if (isset($_GET['id'])) {
			...
			$userNum = M('Users')->where(array('agentid' => intval($_GET['id'])))->count();
		}
		...
		if (isset($_POST['dosubmit'])) {
			...
			else if (substr($_POST['siteurl'], -1, 1) == '/') {
			...
			if ($this->agent_db->create()) {
				if ($thisAgent) {
					if (isset($_POST['is_package']) && (0 < $userNum)) {
						unset($_POST['is_package']);
					}

					if (($thisAgent["is_tongji"] == "1") && ($_POST["is_tongji"] == "0")) {
						$this->closeTongji($thisAgent["id"]);
					}
			...
			$thisAgent['is_package'] = isset($thisAgent['is_package']) ? $thisAgent['is_package'] : '1';
	...
	private function closeTongji($agentId)
	{
		$userGroupModel = M("UserGroup");
		$groups = $userGroupModel->where(array("agentid" => $agentId, "tj_status" => "1"))->field("id")->select();
		$groupIds = array();
		...
		if (!empty($groupIds)) {
			$usersModel = M("Users");
			$users_ids = array();
			$userGroupModel->where(array(
	"id" => array("in", $groupIds)
	))->save(array("tj_status" => "0"));
			$users = $usersModel->where(array(
	"gid" => array("in", $groupIds)
	))->field("id")->select();
