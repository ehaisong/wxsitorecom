...
class UserAction extends BaseAction
{
		...
		if ('Upyun' != MODULE_NAME && 'Attachment' != MODULE_NAME) {
			if (isset($_SESSION['role_name']) && session('role_name') == 'staff' && !IS_AJAX) {
				...
				if (session('uid') == false) {
					if (0 < $is_syn) {
						die('<p class="session-timeout">您的登录超时，请刷新页面。</p>');
					} else {
		...
		if (ALI_FUWU_GROUP) {
			...
			foreach ($fuwuuserlist as $fuwuvo) {
				...
				if ($fuwuuserinfo == '') {
					...
					$foid = D('Userfollow')->setsubscribe($this->token, $fuwuvo['wecha_id'], 1);
	...
	protected function dwzQuery($data)
	{
		...
		if (in_array($url['host'], $urls)) {
			...
			if (in_array(dirname($url['path']), $urls)) {
				return true;
			} else {
				if (in_array($data['tinyurl'], $urls)) {
					return true;
				} else {
					return false;
				}
