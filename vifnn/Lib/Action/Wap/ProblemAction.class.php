...
class ProblemAction extends WapAction {
	...
	public function _initialize() {
		...
		if (empty($this->wecha_id)) {
			$this->wecha_id = 'onfo6t15P570jYq2JMIdaGK7KLaE';
	...
	public function rank() {
		...
		$rank = M('Problem_user')->where($where)->order('score_count desc,expend_count asc')->limit($this->pro_info["rank_num"])->select();
	...
	*/
	public function _rand_question($id, $uid) {
		...
		/***取出还没有回答的题目id***/
		$noanswerId = array_diff($id_arr, $dff);

		$isNoAnswerId = false; /****是否所有题目都回答过了***/
		if (empty($noanswerId)) {
			$key = array_rand($id_arr);
			$isNoAnswerId = true;
		} else {
			$key = array_rand($noanswerId);
		}
		...
		/***取出还有没回答到的题目的题目id****/
		if (in_array($id_arr[$key], $question_id_session) && !$isNoAnswerId) {
