...
class CardAction extends WapAction
{
	...
	public function addSign()
	{
		...
		if ($rt) {
			...
			$arr['staffid'] = 0;
			$arr['notes'] = '签到赠送积分';
			$arr['score'] = $data['expense'];
			$arr['cardid'] = intval($_GET['cardid']);
	...
	public function jfdhhb()
	{
		if (IS_AJAX) {
			...
			if ($jfdhhb['endtime'] < time()) {
				...
				if (($this->wecha_id == '') || ($userinfo == '')) {
					$data['error'] = 1;
					$data['msg'] = '对不起，获取不到您的个人信息。';
				}
				else if ($jfdhhb['count'] <= $my_count) {
					$data['error'] = 1;
					$data['msg'] = '对不起，您已经兑换了' . $jfdhhb['count'] . '次，不能再兑换了。';
				}
				else if ($my_integral < $jfdhhb['integral']) {
					$data['error'] = 1;
					...
					if (strtoupper($res['status']) == 'FAIL') {
						...
						else if (!(strpos($res['msg'], '过于频繁') === false)) {
					...
					else if (strtoupper($res['status']) == 'SUCCESS') {
						...
						$arr['staffid'] = 0;
						$arr['notes'] = '兑换红包';
						$arr['score'] = $jfdhhb['integral'] * -1;
						$arr['cardid'] = intval($jfdhhb['cardid']);
						M('Member_card_use_record')->add($arr);
			...
			$this->assign('thisCard', $thisCard);
			$jfdhhb_list = M('member_card_jfdhhb')->where(array(
	'token'     => $this->token,
	'cardid'    => intval($_GET['cardid']),
	'display'   => 2,
	'starttime' => array('lt', time())
	))->order('hongbao')->select();
