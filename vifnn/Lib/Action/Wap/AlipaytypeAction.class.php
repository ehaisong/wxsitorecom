	...
	public function call_back_url(){
		...
		if($verify_result) {//验证成功
			...
			if($result == 'success') {
				...
				// 2015-05-14 lixiang 支付成功后跳出iframe 如果没有iframe侧直接跳转
				...
				//header('location: /index.php?g=Wap&m='.$from.'&a=payReturn&token='.$orderInfo['token'].'&wecha_id='.$orderInfo['wecha_id'].'&orderid='.$out_trade_no);
			}else {
			  exit('付款失败'); 
			}
		}
