...
class ServiceAction extends WapAction
{
	...
	public function _initialize()
	{
		...
		$serviceChange->index($this->token, $this->siteUrl);
	}
	...
	public function chat()
	{
		...
		if ($mywxuser['app_id'] == '') {
			...
			$data_s['domain'] = str_replace('http://', '', $this->siteUrl);
			$data_s['domain'] = str_replace('https://', '', $data_s['domain']);
			$data_s['label'] = $this->token;
			$data_s['wx_app_id'] = $wxuser['appid'];
			$data_s['wx_app_secret'] = $wxuser['appsecret'];
			$data_s['from'] = '1';
			$data_s['activity_url'] = $this->siteUrl . '/index.php?g=Wap&m=ServiceReturn&a=activity&token=' . $this->token;
			$data_s['msg_tip_url'] = $this->siteUrl . '/index.php?g=Wap&m=ServiceReturn&a=msgtip&token=' . $this->token;
			$data_s['my_url'] = $this->siteUrl . '/index.php?g=Wap&m=ServiceReturn&a=my&token=' . $this->token;
			ksort($data_s);
			$i = 0;
			...
			$data_s['key'] = md5($mydata);
			$open = json_decode($this->https_request($this->apiurl['create'], $data_s), true);
			$where_mywxuser['token'] = $this->token;
			$save_mywxuser['state'] = $_GET['state'];
			$save_mywxuser['app_id'] = $open['app_id'];
			$mywxuser['app_id'] = $open['app_id'];
			$save_mywxuser['app_key'] = $open['app_key'];
			$mywxuser['app_key'] = $open['app_key'];
	...
	public function servicesay($to_openid, $from_nickname, $url)
	{
		...
		$access_token = $apiOauth->update_authorizer_access_token($appid, $wxuser);
