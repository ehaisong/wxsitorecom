...
class SentimentAction extends WapAction
{
	...
	public function index()
	{
		...
		if (($_GET['tel'] != '') && ($this->wecha_id != '')) {
			$userinfo_tel = M('userinfo')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id))->save(array('tel' => $_GET['tel']));
		...
		if (($this->Sentiment['is_attention'] == 1) && !$this->isSubscribe()) {
			...
			if ((($this->Sentiment['is_reg'] == 1) || ($this->Sentiment['is_sms'] == 1)) && empty($this->fans['tel'])) {
				...
				if (($this->Sentiment['is_sms'] == 1) && ($my['tel'] == 0)) {
					$this->assign('sms', 1);
					$this->assign('memberNotice', '<div style="display:none"></div>');
				}
			}
	...
	{
		if ($_POST['tel'] != '') {
			$is_tel = M('Sentiment_user')->where(array('pid' => intval($_POST['id']), 'token' => $_POST['token'], 'tel' => $_POST['tel']))->find();
