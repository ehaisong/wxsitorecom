<?php
	class SeckillAction extends WapAction {
		...
		public function shop() {
			...
			$thisshop_num=S('shop_num_'.$sid);
			
			if(is_bool($thisshop_num) || (!($thisshop_num>0) && (intval($thisshop_num)!=-1))){
			  S('shop_num_'.$sid,intval($data['shop_num']),100);
			}
		...
		public function buy() {
			if(IS_POST) {
				...
				****************************************************************************/
				...
				$shop_num = S('shop_num_'.$sid);
				if( isset($shop_num) && $shop_num <= 0) {	
					echo json_encode( array(
						"status" => -1,
						"msg" => "抢光啦！"
						) ) ;
					...
					/*$newshop_num=$shop_num-1;
					$newshop_num=$newshop_num>0 ? $newshop_num :'-1';
					S('shop_num_'.$sid,$newshop_num,100);*/
					echo json_encode( array(
						"status" => 0,
						"msg" => "ok"
						));
		...
		public function pay() {
			if( IS_POST ){
				...
				if(($shop_num < 0) || !($seckill_base_shop['shop_num']>0)){
					$this->error("货品已被抢光") ;
				}
				...
				$bookorder = M('seckill_book')->where(array('orderid'=>$orderid,'wecha_id'=>$this->wecha_id))->find();
				if(empty($bookorder)){
				   $this->error("订单已经不存在了！") ;
				}
				//当前商品基本信息
				$seckill_base_shop = M("seckill_base_shop")->field("shop_num")->where( array("shop_id" =>$bookorder['book_sid'] ,"shop_open" => 0))->find() ;
				if(empty($seckill_base_shop)){
				   $this->error("您所购买的商品已经下架了！") ;
				}elseif(!($seckill_base_shop['shop_num']>0)){
				  $this->error("商品已被抢光了") ;
				}
