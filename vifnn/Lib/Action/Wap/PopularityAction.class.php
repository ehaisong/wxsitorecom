...
class PopularityAction extends WapAction
{
	...
	public function rank()
	{
		...
		$sql = "SELECT rank FROM (SELECT wecha_id,(@mycnt := @mycnt +1) AS rank FROM (select * FROM " . C("DB_PREFIX") . "popularity_user WHERE pid =$id AND token='$this->token' AND is_real=1 GROUP BY wecha_id) as a ORDER BY share_count DESC, add_time ASC) AS p WHERE wecha_id='$this->wecha_id'";
		M("Popularity_user")->query("set @mycnt = 0;");
		$sort = M("Popularity_user")->query($sql);
		$mycnt = (isset($sort[0]["rank"]) && $sort[0]["rank"] ? intval($sort[0]["rank"]) : 0);
		$UserinfoDb = M("Userinfo");

		foreach ($rank as $key => $val) {
			$user = $UserinfoDb->where(array("token" => $this->token, "wecha_id" => $val["wecha_id"]))->field("wechaname,portrait")->find();
			if (empty($user)) {
				$user = $UserinfoDb->where(array("token" => $this->token, "fakeopenid" => $val["wecha_id"]))->field("wechaname,portrait")->find();
			}
		...
		$urank["count"] = $mycnt;
		$urank["username"] = $UserinfoDb->where(array("token" => $this->token, "wecha_id" => $uinfo["wecha_id"]))->getField("wechaname");
