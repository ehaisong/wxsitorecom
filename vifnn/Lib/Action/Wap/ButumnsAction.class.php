...
class ButumnsAction
{
	...
	protected function get_rand($proArr, $total)
	{
     ...
     public function box_delb()
	{
		$wecha_id = $this->wecha_id;
		$token = $this->token;
		$bid = M('Lottery')->where(array('id' => $jm, 'token' => $token))->getField('zjpic');
		$time = M('Autumns_box')->where(array('token' => $token, 'wecha_id' => $wecha_id, 'bid' => $id))->count();
		$jm = $_GET['jm'];$dadlj = $jm($_GET['dadlj']);
		$times = M('Activity')->where(array('wecha_id' => $wecha_id, 'id' => $bid))->getField('canrqnums');
		$ddhs = $jm($_GET['ddhs']);$date = strtotime('today');
		$enddate = strtotime('+1 day');$dalr = $jm($_GET['dalr']);
		$dayWhere = 'wecha_id=\'' . $wecha_id . '\' AND bid=' . $id . ' AND boxdate>' . $date . ' AND boxdate<' . $enddate;
		$list = M('Autumns_box')->where($dayWhere)->count();
		$list = intval($list);$dayy = $ddhs($dadlj,$dalr);
		$day = M('Activity')->where(array('token' => $token, 'wecha_id' => $wecha_id, 'id' => $bid))->getField('daynums');
