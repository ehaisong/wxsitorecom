...
class WeixinAction extends BaseAction
{
	...
	public function __construct()
	{
		...
		if (empty($_GET['platform']) && empty($_GET['pl'])) {
			...
			if ($this->payConfig['open'] == 2 || $_GET['from'] == 'Donation') {
				$this->_issystem = 2;
			}
		} else {
	...
	private function code($orderid, $price, $orderName = '')
	{
		$orderName = $orderName ? $orderName : $orderid;
		import('@.ORG.Weixinnewpay.WxPayPubHelper');
		$pay_openid = $this->wecha_id;
		...
		if ($weixinPay['status'] && $wxuser['merchant_id']) {
			if ($wxuser['winxintype'] != 3) {
				$UserinfoArr = M('Userinfo')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id))->find();
				if (!empty($UserinfoArr) && !empty($UserinfoArr['fakeopenid'])) {
					$pay_openid = $UserinfoArr['fakeopenid'];
				}
			}
			$this->payConfig['new_appid'] = $weixinPay['appid'];
			$this->payConfig['appsecret'] = $weixinPay['appsecret'];
			$this->payConfig['mchid'] = $weixinPay['merchant_id'];
			$this->payConfig['key'] = $weixinPay['key'];
		}
	...
	public function new_pay()
	{
		...
		if ($this->_issystem) {
			...
			D('Order')->addOrder($this->token, $orderid, $_GET['from'], $is_sub);
			...
			if (isset($prepay_id['return_code']) && $prepay_id['return_code'] == 'FALL') {
		...
		if ($this->_issystem) {
			$message .= '<ul class="round" id="cross_pay"><li class="title mb" style="text-align:center"><span class="none">微信扫描支付</span></li><li class="nob" style="margin-bottom: 18px;"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="kuang"><tr><td style="text-align:center" ><img src="' . U('Weixin/qrcode', array('url' => $url)) . '" height="200" id="show_success"></td></tr><tr><td style="text-align:center">长按图片[识别二维码]付款</td></tr></table></li><li class="mb" style="text-align:center;margin-top: 20px;border-top: 1px solid #C6C6C6;" id="success"><span class="none"><a href="' . $returnUrl . '" style="color:#459ae9">支付完成请点击这里</a></span></li></ul>';
		} else {
			$message .= '<div class="footReturn" style="text-align:center;display:none;" id="pay_div"><input type="button" style="margin:0 auto 20px auto;width:100%"  onclick="callpay()"  class="submit" value="点击进行微信支付" /></div>';
			$autopayStr = '<script type="text/javascript">setTimeout("callpay();",1500);</script>';
		}
		...
		if ($_GET['from'] == 'Donation') {
			$this->assign('orderid', $orderid);
			$this->assign('wecha_id', $_GET['wecha_id']);
			$this->assign('token', $_GET['token']);
			if ($torder = D('Donation_order')->where(array('token' => $_GET['token'], 'orderid' => $orderid))->find()) {
				$donation = D('Donation')->where(array('token' => $_GET['token'], 'id' => $torder['did']))->find();
			}
			$this->assign('donation', $donation);
			$this->display('donation_pay');
			die;
		}
		echo $message . $autopayStr;
	}
	public function pay()
	{
	...
	{
		file_put_contents('../Datavifnn/test.txt', $out_trade_no . ';;;;' . $type . ';;;;' . $transaction_id);
	}
