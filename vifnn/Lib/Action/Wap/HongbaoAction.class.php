...
class HongbaoAction extends WapAction
{
	...
	public function index()
	{
		...
		if ($cache_action != '') {
			...
			$action = M('hongbao')->where(array('id' => $this->action_id))->find();
		...
		if ($this->wecha_id) {
			if (empty($this->my_packets)) {
				...
				$savedata = array("grabber_nickname" => !empty($this->fans["wechaname"]) ? $this->fans["wechaname"] : "匿名", "grabber_headimgurl" => !empty($this->fans["portrait"]) ? $this->fans["portrait"] : $this->siteUrl . "tpl/User/default/common/images/portrait.jpg", "grabber_sex" => !empty($this->fans["sex"]) ? $this->fans["sex"] : 0, "grabber_tel" => !empty($this->fans["tel"]) ? $this->fans["tel"] : "", "grabber_qq" => !empty($this->fans["qq"]) ? $this->fans["qq"] : "", "grabber_address" => !empty($this->fans["address"]) ? $this->fans["address"] : "", "grabber_province" => !empty($this->fans["province"]) ? $this->fans["province"] : "", "grabber_city" => !empty($this->fans["city"]) ? $this->fans["city"] : "");
				M("hongbao_grabber")->where(array("hongbao_id" => $this->action_id, "grabber_wechaid" => $this->wecha_id))->save($savedata);
			}
		}
	...
	public function grab_packet()
	{
		...
		$action = M("hongbao")->where(array("id" => $this->action_id, "status" => "1"))->find();
		...
		$already_join = M("hongbao_grabber")->where(array(
	"hongbao_id" => $action["id"],
	"isgrabbed"  => array("neq", "0")
	))->count();
		if (($action["limit_join"] <= $already_join) && ($action["limit_join"] != 0)) {
			$this->error("抢红包人数超出限制!");
		}
		...
		$update_hongbao = M('hongbao')->where(array('id' => $this->action_id))->save($hb_d);
		...
		if ($this->action_id && $this->token) {
			...
			if (!empty($share)) {
				if ($share['is_opened'] == 1) {
					...
					$hb_d = array();
					$hb_d['total_money'] = array('exp', 'total_money-' . $rand_money);
					$u_hongbao = M('hongbao')->where(array('id' => $this->action_id))->save($hb_d);
	...
	public function common_packets()
	{
		...
		$display_nums = M("hongbao")->where(array("id" => $this->action_id))->getField("display_nums");

		if (0 < $display_nums) {
			$lucky_guys = M("hongbao_grabber")->where("hongbao_id = $this->action_id")->order("money desc")->limit(0, $display_nums)->select();
		}
		else {
			$lucky_guys = M("hongbao_grabber")->where("hongbao_id = $this->action_id")->order("money desc")->limit(0, 100)->select();
		}
	...
	public function notice($actioninfo = '')
	{
		...
		if (($actioninfo['need_follow'] == 1) && ($this->isSubscribe() == false)) {
			...
			if (($actioninfo['need_phone'] == 1) && empty($this->fans['tel'])) {
				...
				$this->assign('link', $this->getLink($actioninfo['remind_link']));
	...
	{
		$total_grabber = M("hongbao_grabber")->where("hongbao_id = $this->action_id AND isgrabbed != 0")->count();
		exit($total_grabber);
	}
}
