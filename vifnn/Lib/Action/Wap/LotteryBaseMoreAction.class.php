...
class LotteryBaseMoreAction extends WapAction
{
	...
	'and'
				...
				if (($Lottery['isdaylottery'] == 1) && (1 <= $thisDayLotteryNum)) {
					...
					case 2:
						$shareWhere = 'token=\'' . $token . '\' AND wecha_id=\'' . $wecha_id . '\' AND lid=' . $id . ' AND sharecount>0';
						$thisShareCount = $redata->where($shareWhere)->getField('sharecount');
						$thisShareCount = ($thisShareCount ? $thisShareCount : 0);
						$playcount = floor($thisShareCount / $Lottery['shareupnum']) + $Lottery['sharebeforenum'];
						if (($playcount <= $record['usenums']) && $Lottery['shareupnum']) {
							$isnum = 2;
						}
						else {
							$isnum = 0;
					...
					if ($isnum) {
						...
						case 2:
							$data['end'] = -3;
							$data['zjl'] = 0;
							$data['winprize'] = '还要分享' . (((($record['usenums'] - $Lottery['sharebeforenum']) + 1) * $Lottery['shareupnum']) - $thisShareCount) . '次才能抽奖哦~';
							$data['sharediff'] = ((($record['usenums'] - $Lottery['sharebeforenum']) + 1) * $Lottery['shareupnum']) - $thisShareCount;
							break;
						}
					}
					else {
						...
						if ((0 < $Lottery['timespan']) && ($timecha < ($Lottery['timespan'] * 60))) {
							...
							if (($Lottery['fistlucknums'] == $Lottery['fistnums']) && ($Lottery['secondlucknums'] == $Lottery['secondnums']) && ($Lottery['thirdlucknums'] == $Lottery['thirdnums']) && ($Lottery['fourlucknums'] == $Lottery['fournums']) && ($Lottery['fivelucknums'] == $Lottery['fivenums']) && ($Lottery['sixlucknums'] == $Lottery['sixnums'])) {
								...
								if ($cheat) {
									$lottery_num = array(1 => 'fist', 2 => 'second', 3 => 'third', 4 => 'four', 5 => 'five', 6 => 'six');

									if ($Lottery['' . $lottery_num[$cheat['prizetype']] . 'nums'] <= $Lottery['' . $lottery_num[intval($cheat['prizetype'])] . 'lucknums']) {
										$prizeType = 7;
									}
									else {
						...
						if ($isLottery && $check) {
							...
							$rid = $this->lottory_record_db->add(array('token' => $token, 'wecha_id' => $wecha_id, 'lid' => $id, 'usenums' => $record['usenums'], 'islottery' => $isLottery, 'wecha_name' => '', 'phone' => '', 'sn' => $data['sncode'], 'time' => $now, 'prize' => intval($data['prizetype']), 'sendtime' => 0, 'sharecount' => $record['sharecount']));
						}
