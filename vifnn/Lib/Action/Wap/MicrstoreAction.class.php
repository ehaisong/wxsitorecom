	...
	public function pay()
	{
		...
		if(!$_GET['wecha_id']){
			if($this->wxuser['winxintype'] == 3){
				...
				$_GET["wecha_id"] = (!empty($this->wecha_id) ? $this->wecha_id : $this->fakeopenid);
			}
		}
		...
		$data = array(
			...
			'notOffline' => $_GET['notOffline'],
		);
		$data['salt'] = $this->SALT;
		ksort($data);
		$fake_data 	= $data;
		$fake_data['wecha_id'] 	= $this->fakeopenid; 

		$flag 		= true;
		$sign_key 	= sha1(http_build_query($data));
		$fake_key 	= sha1(http_build_query($fake_data));

		if ($sign_key != $_GET['sign_key'] && $fake_key != $_GET['sign_key']) {
			$flag 	= false;
		}
