...
class RepastAction extends WapAction
{
	...
	public function dishMenu()
	{
		...
		if (!empty($errorstr) && (!isset($outset['starttime2']) || !(0 < $outset['starttime2']))) {
			$this->error($errorstr);
			exit();
		}
		...
		if (isset($outset['starttime2']) && (0 < $outset['starttime2'])) {
			...
			if (empty($outset['starttime']) && ($setime < $nows)) {
				$errorstr = '';
			}
	...
	'isopen' => 1
			...
			$this->checkInstock($tmparr, $shopid);
	...
	'd_t.isuse'       => array('neq', 2)
		...
		if ($diningTable && (0 < $diningTable['status'])) {
		...
		if ($table) {
			foreach ($table as $tkk => $tvv) {
				if ($reservetids && in_array($tvv['id'], $reservetids)) {
					...
					if (($tvv['status'] == 1) && ($selecttime < $twohours)) {
						$table[$tkk]['statusStr'] = '使用中';
					}
					else {
						$table[$tkk]['statusStr'] = '';
	...
	public function saveOrderAndToPay()
	{
		...
		if (is_array($tmpOrderdata)) {
			...
			$sessionorid = $_SESSION[$sessionoridK];
			$allmark = $_SESSION['allmark' . $this->_cid . $this->token];
			$this->checkInstock($tmpOrderdata['orderdish'], $this->_cid, true);
			if ((0 < $orid) && ($orid == $sessionorid)) {
				$takeaway = 2;
				...
				$printtype = (!(0 < $isallpay) ? '点餐预定金' : '餐饮付款');
			}
...
*/
				...
				if ($alipayConfig['open']) {
					...
					if (isset($isallpay) && ($isallpay == 0) && (0 < $DishC['advancepay'])) {
						$paydata['ydMeal'] = 1;
					}
		...
		if (0 < $tkconoff) {
			...
			foreach ($dishArr as $kkd => $vvd) {
				...
				if (!(0 < $tmpdish['instock'])) {
					$this->error('抱歉！' . $vvd['name'] . ' 这道菜今日已经买完了，请重新点菜吧！', U('Repast/dishMenu', array('token' => $this->token, 'cid' => $tcid)));
				}
				...
				if ($tmpdish['instock'] < $tmpinstock) {
					...
					if ($locktime < $nowtime) {
						...
						if ($tmpdish['instock'] < $vvd['num']) {
							$this->error('抱歉！' . $vvd['name'] . ' 这道菜库存不足请少点几份吧！', U('Repast/dishMenu', array('token' => $this->token, 'cid' => $tcid)));
						}
					}
					else {
						$this->error('抱歉！' . $vvd['name'] . ' 库存不足请稍等几分钟重新点菜吧！', U('Repast/dishMenu', array('token' => $this->token, 'cid' => $tcid)));
					}
				}
