...
class TestAction extends WapAction{
	...
	public function index(){
		...
		$where_test['vifnn_id'] = (int)($_GET['id']);
		$test = $this->m_test->where($where_test)->find();
		$save['pv'] = $test['pv'] + 1;
		$update = $this->m_test->where($where_test)->save($save);
		$this->assign("test",$test);
		$where_test_list['token'] = $this->token;
		$test_list1 = $this->m_test->where($where_test_list)->order("vifnn_id desc")->select();
		$test_list2 = $this->m_test->where($where_test_list)->order("vifnn_id")->select();
		if($test_list1[0]['vifnn_id'] == (int)($_GET['id'])){
			$next['id'] = $test_list2[0]['vifnn_id'];
			$next['name'] = $test_list2[0]['name'];
		}else{
			foreach($test_list2 as $k=>$v){
				$test_next[$v['vifnn_id']] = $k;
			}
			$where_test_next['token'] = $this->token;
			$where_test_next['vifnn_id'] = $test_list2[($test_next[(int)($_GET['id'])]+1)]['vifnn_id'];
			$find_test_next = $this->m_test->where($where_test_next)->find();
			$next['id'] = $find_test_next['vifnn_id'];
