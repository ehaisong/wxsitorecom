...
class UnitaryAction extends WapAction {
	...
	public function dobuy() {
		...
		$save_order['orderid'] = date("YmdHis", time()) . rand(10000, 99999); //$id_order."UNITARY".time().$randnum;
	...
	public function nopayover() {
		...
		if (IS_POST && isset($_GET['orderid']) && !empty($_GET['orderid']) && !empty($myorder)) {
			...
			//$this->redirect("Unitary/payend",array("token"=>$this->token,"orderid"=>$myorder['orderid'],'unitaryid'=>$_GET['unitaryid']));
		}
	}
			...
			if ($myorder['addtime'] != 0) {
				...
				foreach ($cart_list as $key => $val) {
					...
					if (intval($val['count_s']) > 0) {
						...
						if ($pay_count >= $unitary['price'] && $unitary['state'] != 2) {
