...
class DonationAction extends WapAction
{
	...
	public function _initialize()
	{
		...
		$this->checkTongji($this->token, "donation", $this->_id);
	}
	...
	public function index()
	{
		...
		if (($donation["need_attention"] == 1) && !$this->isSubscribe()) {
			$this->memberNotice("详细了解捐款信息，请点击关注公众账号。", 1, "", "关注");
		}
		else {
			if (($donation["need_phone"] == 1) && ($this->fans["tel"] == "")) {
				$this->memberNotice("请填写您的手机号。", 0);
			}
		}

		$total_array = $this->total_cnt();
		$count_share = D("Donation_order")->field("count(distinct(wecha_id)) as people, count(distinct (sid)) as share")->where(array(
	"did"   => $this->_id,
	"token" => $this->token,
	"paid"  => 1,
	"sid"   => array("gt", 0)
		...
		if ($share_ids) {
			$count_list_tmp = D("Donation_order")->field("sid, count(distinct (wecha_id)) as cnt, sum(price) as price")->where(array(
	"did"   => $this->_id,
	"token" => $this->token,
	"paid"  => 1,
	"sid"   => array("in", $share_ids)
	))->group("sid")->select();
			$count_list = array();
			...
			$share_list_tmp = D("Donation_share")->where(array(
	"token" => $this->token,
	"id"    => array("in", $share_ids)
	))->select();
			$share_list = array();
	...
	private function check_donation($jump)
	{
		...
		else if (!strstr($donation["reply_pic"], "http://")) {
	...
	"id"    => array("in", $share_ids)
		...
		exit(json_encode(array("data" => $orders, "page" => $page)));
	}
	...
	public function creat_invitation()
	{
		...
		if (empty($content)) {
			exit(json_encode(array("status" => true, "info" => "邀请口号不能为空")));
		}

		if (80 < dstrlen($content)) {
			exit(json_encode(array("status" => true, "info" => "邀请口号不能超过80个汉字")));
		}
		...
		if ($share = D("Donation_share")->where(array("token" => $this->token, "wecha_id" => $this->wecha_id, "did" => $this->_id))->find()) {
			if (D("Donation_share")->where(array("id" => $share["id"]))->save($data)) {
				exit(json_encode(array("status" => false, "data" => $share["id"])));
			}
			else {
				exit(json_encode(array("status" => true, "info" => "分享失败，稍后重试！")));
			}
		}
		else if ($sid = D("Donation_share")->add($data)) {
			exit(json_encode(array("status" => false, "data" => $sid)));
		}
		else {
			exit(json_encode(array("status" => true, "info" => "分享失败，稍后重试！")));
		}
	...
	public function check_pay()
	{
		...
		if (isset($order["paid"]) && $order["paid"]) {
			exit(json_encode(array("status" => 1, "id" => $order["did"])));
		}
		else {
	...
	public function info()
	{
		...
		if ($type == 3) {
			...
			$sql = "SELECT v.num,v.wecha_id, u.portrait, u.wechaname FROM " . C("DB_PREFIX") . "donation_value as v LEFT JOIN " . C("DB_PREFIX") . "userinfo AS u ON u.wecha_id=v.wecha_id WHERE v.token='$this->token' AND u.token='$this->token' AND v.did='$this->_id'  ORDER BY v.num DESC, v.id ASC LIMIT 0, 10";
			$value_list = D()->query($sql);

			if (!empty($value_list)) {
				foreach ($value_list as $ook => &$oo ) {
					if (empty($oo["portrait"]) || empty($oo["wechaname"])) {
						$tmpuserinfo = $this->getuserinfo($oo["wecha_id"]);
						!empty($tmpuserinfo) && empty($oo["portrait"]) && ($oo["portrait"] = $tmpuserinfo["portrait"]);
						!empty($tmpuserinfo) && empty($oo["wechaname"]) && ($oo["wechaname"] = $tmpuserinfo["wechaname"]);
					}
				}
			}
			...
			if ($sids) {
				$share_count = D("Donation_order")->field("count(1) as cnt, sum(price) as price")->where(array(
	"token" => $this->token,
	"sid"   => array("in", $sids),
	"paid"  => 1,
	"did"   => $this->_id
			...
			$sql = "SELECT SUM( price ) AS p,wecha_id,max(id) as oid FROM " . C("DB_PREFIX") . "donation_order  WHERE paid=1 AND token='$this->token' AND did='$this->_id' GROUP BY wecha_id ORDER BY p DESC, oid ASC LIMIT 0, 10";
			...
			if (!empty($order_list)) {
				...
				foreach ($order_list as &$oo ) {
					$oo["medal"] = "";
					$oo["portrait"] = $oo["wechaname"] = "";
					$tmpuserinfo = $this->getuserinfo($oo["wecha_id"]);
					!empty($tmpuserinfo) && ($oo["portrait"] = $tmpuserinfo["portrait"]);
					!empty($tmpuserinfo) && ($oo["wechaname"] = $tmpuserinfo["wechaname"]);
	...
	public function ajax_list()
	{
		...
		if ($type == 3) {
			$sql = "SELECT v.num,v.wecha_id, u.portrait, u.wechaname FROM " . C("DB_PREFIX") . "donation_value as v LEFT JOIN  " . C("DB_PREFIX") . "userinfo AS u ON u.wecha_id=v.wecha_id WHERE v.token='$this->token' AND u.token='$this->token' AND v.did='$this->_id'  ORDER BY v.num DESC, v.id ASC LIMIT $star, $pagesize";
			$list = D()->query($sql);

			if (!empty($list)) {
				foreach ($list as $k => &$ll ) {
					$ll["ranking"] = $star + $k + 1;
					if (empty($ll["portrait"]) || empty($ll["wechaname"])) {
						$tmpuserinfo = $this->getuserinfo($ll["wecha_id"]);
						!empty($tmpuserinfo) && empty($ll["portrait"]) && ($ll["portrait"] = $tmpuserinfo["portrait"]);
						!empty($tmpuserinfo) && empty($ll["wechaname"]) && ($ll["wechaname"] = $tmpuserinfo["wechaname"]);
					}
				}
			}
			else {
			...
			$sql = "SELECT SUM( price ) AS p, wecha_id, max(id) as oid FROM " . C("DB_PREFIX") . "donation_order  WHERE paid=1 AND token='$this->token' AND did='$this->_id' GROUP BY wecha_id ORDER BY p DESC, oid ASC LIMIT $star, $pagesize";
			$list = D()->query($sql);
			...
			if (!empty($list)) {
				...
				foreach ($list as $k => &$ll ) {
					$ll["medal"] = "";
					$ll["portrait"] = $ll["wechaname"] = "";
					$tmpuserinfo = $this->getuserinfo($ll["wecha_id"]);
					!empty($tmpuserinfo) && ($ll["portrait"] = $tmpuserinfo["portrait"]);
					!empty($tmpuserinfo) && ($ll["wechaname"] = $tmpuserinfo["wechaname"]);
		...
		exit(json_encode(array("error" => $error, "page" => $page, "data" => $list)));
	}
}
