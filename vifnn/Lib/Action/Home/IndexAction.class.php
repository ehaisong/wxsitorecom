...
class IndexAction extends BaseAction{
	...
	protected function _initialize(){
		parent::_initialize();
		$this->home_theme=$this->home_theme?$this->home_theme:'vifnn'; //模板目录名字
	...
	function home_ajax(){
		...
		if($list != 1){//list！=1,不是节日营销导航
			...
			if(empty($img_all)){
				$img_all="<li> <img src='./tpl/Home/vifnn/common/images/step/wutu.png'/></li>";
				$one_point.="&nbsp;";
			}
			$data['li']=$caidan_li;
	...
	function five_ajax(){//节日营销右侧点击填充单页面
		...
		if(empty($one_arr['img_all'])){
			$one_arr['img_all']="<li> <img src='./tpl/Home/vifnn/common/images/step/wutu.png'/></li>";
			$one_arr['one_point']="&nbsp;";
		}
	...
	public function qrcodeLogin(){
		if (isset($_SESSION['preuid'])){
			...
			$this->success('现在进入体验',U('User/Index/info'));
		}else {
			$this->success('超时，请联系客服审核账号',U('Home/Index/info'));
		}
	}
	...
	//2015.03.23新增 by：微服私访
        //用户登出
	public function logout(){
		session(null);
		session_destroy();
		unset($_SESSION);
		redirect(U('Home/Index/info'));
	}
	...
	public function agents(){
		...
		$where['id'] = $this->_get('id');
		$agents =M('Agent')->where($where)->find();		
                $content = stripslashes(htmlspecialchars_decode($agents['content']));	
		$this->assign($agents);
		$this->assign('content',$content);		
		$this->display($this->home_theme.':Index:'.ACTION_NAME);
	}	
	...
	public function renewJson(){
	 ...
	 public function userJson(){
    	if (C('server_topdomain')=='vifnn.cn'&&$this->_get('key')==C('server_key')){
    		$id=intval($_GET['id']);
	...
	public function login(){
		...
		$business = include('./vifnn/Lib/ORG/Business.php');
		$i=0;
