...
class UsersAction extends BaseAction{
	...
	public function companylogin() {
		...
		if (empty($k) || $k != md5($where['id'] . $where['username'])) {
			$this->error('帐号密码错误1',U('Home/Index/clogin', array('cid' => $cid, 'k' => $k)));
		}
		...
		if($company && ($pwd === $company['password'])){
			if ($wxuser = D('Wxuser')->where(array('token' => $company['token']))->find()) {
				...
				$this->error('帐号密码错误2',U('Home/Index/clogin', array('cid' => $cid, 'k' => $k)));
			}
			session('companyk', $k);
...
// 				}
    ...
    public function cashierBack() {
        ...
        if ($lgcode) {
            ...
            if (!empty($lgcodeArr) && ($lgcodeArr['ptoken'] == $token) && ($lgcodeArr['mktime'] > $nowtime) && ($lgcodeArr['mid'] > 0)) {
                ...
                if ($towelcome) {
                    /*  * *跳转到相应token功能管理页** */
                    redirect(U('User/Index/info', array('token' => $token)));
                    exit();
                }
            }
        }
	...
	public function checkreg(){
			...
			$this->error('系统已经关闭注册，请联系管理开通！',U('Index/info'));
                }
		//2015.03.23新增 by：微服私访	
		$db=D('Users');
		//
			...
			if($id){
				...
				if($this->reg_needCheck){
					...
					if (C('demo')){
						...
						$this->success('注册成功,请联系在线客服审核帐号',U('User/Index/info'));exit;
					}
				}else{
