...
class WapAction extends BaseAction {
	...
	protected function _initialize() {
		...
		if (ALI_FUWU_GROUP && date('Y') == 2015) {
			...
			foreach ($fuwuuserlist as $fuwuvo) {
				...
				if ($fuwuuserinfo == '') {
					...
					D('Userfollow')->setsubscribe($this->token, $fuwuvo['wecha_id'], 1); //更新关注表
		...
		//if (C('server_topdomain')=='vifnn.cn'&&$this->wxuser['winxintype']==3&&!$this->wxuser['oauth']){
		...
		*/
		...
		if (2 == $users['is_syn']) {
			...
			if (1 == $this->_auth->code) {
				...
				D('Userfollow')->setsubscribe($this->token, $_SESSION[$session_openid_name], 1); //更新关注表
				...
				if (!empty($token_info)) {
					...
					if ($this->wxuser['oauthinfo'] && (MODULE_NAME != 'Index' || (MODULE_NAME == 'Index' && ACTION_NAME == 'content'))) {
						...
						if (isset($jsonui['openid']) && $jsonui['openid']) {
							...
							/*if ($fansInfo) { //用新的wecha_id查一下 防止分享上带wecha_id时其他人点击
								$exist = $fansInfo['id'];
	...
	private function _mergeUserinfo($userinfo, $fakeUserinfo) {
		...
		foreach ($fakeUserinfo as $key => $value) {
			if (empty($fakeUserinfo[$key])) { // 假的为空
				...
				// 假的不为空 判断真是否为空，为空还使用假的 不为空 使用真的
				if ('fakeopenid' == $key) {
					continue;
				}
	...
	public function getQrcode($kword) {
		...
		if ($recognis != '') {
			if ($recognis['code_url'] == '') {
				...
				if($recognis['id']<100000){/*****QR_LIMIT_SCENE为永久二维码*scene_id小于100000****/
			...
			$xid = $recdb->add($dataz);
			$qrcode_url = 'https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=' . $this->access_token;
			if($xid<100000){/*****QR_LIMIT_SCENE为永久二维码*scene_id小于100000****/
				$data['action_name'] = 'QR_LIMIT_SCENE';
