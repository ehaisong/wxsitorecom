...
class ShakeLotteryAction extends UserAction{
		...
		if(IS_POST){
			...
			if(!$id){//添加
				...
				if($edit){
					...
					S('shakelottery_'.$this->token.'_'.$id,null);
	...
	public function DelShakeLottery(){
		...
		if($del){
			...
			$users=M('shakelottery_users')->where(array('aid'=>$id))->field('id,wecha_id,token')->select();
			foreach ($users as $user)
			{
				$shakelottery_user_name='shakelottery_user_'.$id.'_'.$user['token'].'_'.$user['wecha_id'];
				S($shakelottery_user_name,null);
			}
			M('shakelottery_users')->where(array('aid'=>$id))->delete();
			S('shakelottery_'.$this->token.'_'.$id,null);
			S($this->token.'_prizelist_'.$id,null);
	...
	public function AddEditPrize(){
		if(IS_POST){
			...
			if(!$id){//添加
				...
				if($prizeid){
					S($data['token'].'_prizelist_'.$data['aid'],null);
				...
				if($editprize){
					S($this->token.'_prizelist_'.intval($_POST['aid']),null);
	...
	public function DelPrize(){
		...
		if($del){
			S($this->token.'_prizelist_'.$aid,null);
