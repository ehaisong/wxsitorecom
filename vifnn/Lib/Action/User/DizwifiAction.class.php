...
class DizwifiAction extends UserAction
{
	...
	public function AddDevice()
	{
		if (IS_POST) {
			...
			if ($_POST['bssid'] == '') {
				...
				if (!preg_match('/^[A-Fa-f0-9]{2}(:[A-Fa-f0-9]{2}){5}$/', $_POST['bssid'])) {
	...
	public function getcode()
	{
		...
		if (S('shopcode_' . $device['ssid'] . '_' . $device['shop_id']) != '') {
			$this->assign('codeurl', S('shopcode_' . $device['ssid'] . '_' . $device['shop_id']));
			$this->display();
			return true;
		}
		if (!empty($device)) {
			...
			if ($code['errcode'] == 0) {
				S('shopcode_' . $device['ssid'] . '_' . $device['shop_id'], $code['successmsg'], $this->expiration_time);
	...
	public function sethomepage()
	{
		if (IS_POST) {
			...
			if ($_POST['template_id'] == 1 && $_POST['url'] == '') {
				...
				if ($_POST['template_id'] == 1 && $_POST['url'] != '') {
					$url = $this->convertLink(trim($_POST['url']));
				}
			}
			$data['url'] = $url;
