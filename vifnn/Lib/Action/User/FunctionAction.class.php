...
class FunctionAction extends UserAction
{
	...
	'where' => array('token' => $this->token)
		...
		if (!C('agent_version')) {
			...
			$group = M('User_group')->field('id,name,func')->where('status=1 AND agentid=' . $agentid . ' AND id = ' . session('gid'))->order('id ASC')->find();
	...
	public function show_ajax()
	{
		...
		$this->assign('list', $this->filterSiteName($list));
	...
	{
		foreach ($list as $key => $value) {
			$list[$key]['intro'] = $this->filterStr($list[$key]['intro']);
			$list[$key]['content'] = $this->filterStr($list[$key]['content']);
			$list[$key]['desc'] = $this->filterStr($list[$key]['desc']);
			$list[$key]['sce_content'] = $this->filterStr($list[$key]['sce_content']);
		}
