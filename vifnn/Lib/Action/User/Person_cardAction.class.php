...
class Person_cardAction extends UserAction{
	...
	public function index(){
       ...
       import('ORG.Util.Page');// 导入分页类
        ...
        if($p){
            $url = $url.'&p='.$p; //$this->serverUrl.'/index.php?m=Api&c=card&a=lists&uid='.$uid.'&p='.$p;
            $rt = $this->api_notice_increment($url,array(),'get');
            $cards = json_decode($rt,true);
        }
    ...
    public function weimp_add(){
        if (IS_POST) {
            ...
            $data['position'] = $this->_post('position');//职位
            $data['company'] = $this->_post('company');//公司
			$data['wxname'] = $this->_post('wxname');//微信号
