...
class AlipayAction extends UserAction{
	public function index(){
		...
		if (C('agent_version')){
			...
			$group=$userGroup->field('id,name,price')->where('price>0 AND agentid='.$agentid.' AND status=1')->select();
		}else {
			$group=$userGroup->field('id,name,price')->where('price>0 AND status=1')->select();
		}

		$current_group_price = M('User_group')->where(array('id' => intval($_SESSION['gid'])))->getField('price');

		if ($group){
			$i=0;
			foreach ($group as $g){
				if ($_SESSION['gid']==$g['id']||$current_group_price >= $g['price']){
					unset($group[$i]);
				}
				$i++;
			}
		...
		$staytime= ( $this->user['viptime']-time() ) > 0 ? $this->user['viptime']-time()  : 0;
	...
	public function vip(){
		if (C('agent_version')){
			$agentid=$this->thisAgent['is_package']=='0'?'0':$this->agentid;
			$group=M('User_group')->field('id,name,price')->where('price>0 AND agentid='.$agentid.' AND status=1')->select();
		}else {
			$group=M('User_group')->field('id,name,price')->where('price>0'.' AND status=1')->select();
		}
		$user=M('User_group')->field('price')->where(array('id'=>session('gid')))->find();
		$this->assign('group',$group);
		$this->assign('user',$user);
	...
	public function vip_post (){
		if($_POST['price'] <= 0){
			$this->error('没有价格',U('User/Alipay/index'));exit;
		}
		if (C('agent_version')){
	...
	public function setconfig(){
		...
		$alipay_config['cacert']    = getcwd().'\\vifnn\\Lib\\ORG\\Alipay\\cacert.pem';
