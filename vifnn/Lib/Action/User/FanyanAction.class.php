...
class FanyanAction extends UserAction{
	...
	public function _initialize() {
		...
		parent::_initialize();
		$this->canUseFunction('Fangyan');
		$token_open=M('token_open')->field('queryname')->where(array('token'=>session('token')))->find();

		//if(!strpos($token_open['queryname'],'Fangyan')){

            	//$this->error('9您还开启该模块的使用权,请到功能模块中添加',U('Function/index',array('token'=>session('token'),'id'=>session('wxid'))));
