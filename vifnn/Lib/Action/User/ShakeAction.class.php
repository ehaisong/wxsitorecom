...
class ShakeAction extends UserAction{
	...
	public function add(){
		if (IS_POST){
			...
			$row['time']=time();
			//$this->other_source_data_model($row);
	...
	public function edit(){
		if (IS_POST){
			...
			//$this->delete_other_source_data_model($row);
			...
			$shake_act_name=sha1('shake_act_'.$updateWhere['id'].'_'.$this->token);
			S($shake_act_name,null);
	...
	public function del(){
		...
		if ($rt){
			...
			$users=M('shake_rt')->where(array('token'=>$this->token,'shakeid'=>$this->token_where['id']))->field('wecha_id')->select();
			foreach ($users as $user)
			{
				$shake_user_name=sha1('shake_user_'.$shake_id.'_'.$user['wecha_id']);
				S($shake_user_name,null);
			}
