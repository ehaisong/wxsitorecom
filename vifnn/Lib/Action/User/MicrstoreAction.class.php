...
class MicrstoreAction extends UserAction
{
	...
	public function _initialize()
	{
		...
		if (updateSync::getIfWeidian()) {
			$this->Micrstore_URL = C('weidian_domain') ? C('weidian_domain') : 'http://wd.sitore.com';
			$this->SALT = C('encryption') ? C('encryption') : 'sitore.com';
		}else {
			$this->Micrstore_URL = 'http://wd.sitore.com';
			$this->SALT = 'sitore.com';
		}
	}
	...
	public function withdraw()
	{
		...
		if ($return['error_code'] == 0) {
			...
			foreach ((array) $return['withdrawals'] as $key => $val) {
				$insert_data['vifnn_id'] = $val['vifnn_id'];
				...
				$exists = M('microsoft_withdraw')->where(array('vifnn_id' => $val['vifnn_id']))->find();

				if ($exists) {
					$update = M('microsoft_withdraw')->where(array('vifnn_id' => $val['vifnn_id']))->save($insert_data);
				}else {
	...
	public function edit_withdraw()
	{
		if (IS_POST) {
			...
			$vifnn_id = $this->_post('vifnn_id', 'intval');
			...
			if (empty($status) || empty($vifnn_id)) {
