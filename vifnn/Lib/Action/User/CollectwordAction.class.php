...
class CollectwordAction extends UserAction{
	...
	public function set(){
		if($this->wxuser['oauth'] != 1 && $this->wxuser['is_syn'] != 2){
			$this->error('本活动必须开启网页授权！',U('User/Auth/index',array('token'=>$this->token)));
		}elseif($this->wxuser['oauthinfo'] != 1 && $this->wxuser['is_syn'] != 2){
	...
	public function del(){
		...
		if(M('Collectword')->where(array('token'=>$this->token,'id'=>$id))->delete()){
			//M('Sentiment_user')->where(array('token'=>$this->token,'pid'=>$id))->delete();
