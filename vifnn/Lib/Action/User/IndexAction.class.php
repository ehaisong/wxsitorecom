...
//企秀同步
	public function eqx(){
				...
				if (empty($eqxuser)){
				   ...
				   if ($b){
				     ...
				     $a=$db->save($date);
				     if($a){$this->success('帐号同步成功',U('Eqx/index'));}else{$this->error('帐号同步失败,请检测结盟表字段是否正常',U('Eqx/keywordlist'));}
				   }else{
                      $this->error('帐号同步失败,请检测结盟总后台场景相关信息是否填写正确',U('Eqx/keywordlist'));
				   }
				}else{
					$db=M('Wxuser');
				     $date['id']=$_GET['id'];
	...
	public function bind()
	{
		...
		if ($account && !empty($account['appId']) && !empty($account['appSecret']) && !empty($account['token']) && !empty($account['encodingAesKey'])) {
			...
			if ($count == 1) {
				...
				redirect($oauthUrl . urlencode('&ac_id=' . $ac_id));
				die;
			} else {
				echo 'error';
			}
	...
	public function ajax_help($group, $module, $action)
	{
		...
		$data = array('key' => C('server_key'), 'domain' => C('server_topdomain'), 'is_text' => $textHelp, 'data_g' => $data_g, 'data_m' => $module, 'data_a' => $action);
		if (!C('emergent_mode')) {
			if (function_exists('curl_init')) {
				...
				$opts = array('http' => array('method' => 'GET', 'timeout' => 4));
