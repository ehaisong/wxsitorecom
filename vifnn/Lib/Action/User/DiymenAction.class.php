...
class DiymenAction extends UserAction
{
	...
	public function wxSuCaiUrl($wxscurl)
	{
		if (strpos($wxscurl, "weixin.qq.com") && strpos($wxscurl, "tempkey=")) {
			$wxsucaiArr = parse_url($wxscurl);
			parse_str($wxsucaiArr["query"], $output);
			$wxsucaiurl = "http://mp.weixin.qq.com" . $wxsucaiArr["path"] . "?";

			foreach ($output as $kk => $vv ) {
				if ($kk == "tempkey") {
					$wxsucaiurl .= $kk . "=" . urlencode($vv) . "&";
					$wxsucaiurl = str_replace("+", "%2B", $wxsucaiurl);
				}
				else {
					$wxsucaiurl .= $kk . "=" . $vv . "&";
				}
			...
			isset($wxsucaiArr["fragment"]) && !empty($wxsucaiArr["fragment"]) && ($wxsucaiurl .= "#" . $wxsucaiArr["fragment"]);
			return $wxsucaiurl;
		}
	...
	'query' => array('wecha_id' => '{wechat_id}')
				...
				$_POST["url"] = $this->wxSuCaiUrl($_POST["url"]);
			}
	...
	public function del()
	{
		...
		$del_url = 'https://api.weixin.qq.com/cgi-bin/menu/delete?access_token=' . $this->access_token;
		$del_return = json_decode($this->https_request($del_url), true);

		if ($del_return['errcode'] == 0) {
			M('conditionalmenu')->where(array('token' => $this->token))->delete();
			M('conditionalmenu_class')->where(array('token' => $this->token))->delete();

			if ($model['fuwuappid'] == '') {
				M('diymen_class')->where(array('token' => $this->token))->delete();
			}

			$this->success('删除成功');
		}
		else {
			$this->error('删除失败！错误信息：' . $del_return['errmsg']);
		}
	}
