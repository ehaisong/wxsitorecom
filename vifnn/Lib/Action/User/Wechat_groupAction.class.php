...
class Wechat_groupAction extends UserAction{
	...
	function groupSet(){
		...
		if(mb_strwidth($this->_post('name'),'UTF-8')>12){
   ...
   public function export_excel_zip(){
		  ...
		  if (!file_exists($zipname)){
			if($zip->open($zipname,ZipArchive::OVERWRITE)){//创建一个空的zip文件
				...
				if($zip->close()){
					for($j=1;$j<=$page;$j++){
						unlink(RUNTIME_PATH.'member_'.$token."_info".date('Ymd')."_{$j}.xls");//删除临时文件
					}
