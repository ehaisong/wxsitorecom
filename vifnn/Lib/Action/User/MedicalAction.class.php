...
class MedicalAction extends UserAction{
            ...
            if($medicalSet == NULL){
                      if($data->create()!=false){
                        ...
                        if($id=$data->data($_POST)->add()){
                                ...
                                $this->handleKeyword($id,'medicalSet',$this->_post('keyword','trim'));
                               // M('Keyword')->add($data1);
            ...
            $wh = array('token'=>session('token'),'id'=>(int)$_id);
             if($data->where($wh)->save($_POST)){
                    ...
                    $da['keyword']=trim($this->_post('keyword'));
                     $this->handleKeyword($_id,'medicalSet',$this->_post('keyword','trim'));
    ...
    public function addtypeSava(){
        ...
        if($id){
            ...
            if($vifnn_Medical_addtype->where(array('id'=>(int)$_POST['id']))->save($savedata)){
            ...
            if($vifnn_Medical_addtype->add($adddata)){
    ...
    public function Level(){
        $vifnn_Medical_Level=M("Medical_level");
        $count= $vifnn_Medical_Level->where(array('token'=>trim($_GET['token'])))->count();
        $Page = new Page($count,10);
        $show = $Page->show();    
        $res = $vifnn_Medical_Level->where(array('token'=>trim($_GET['token'])))->limit($Page->firstRow.','.$Page->listRows)->order('lid desc')->select();
    ...
    public function LevelAdd_save(){
        $vifnn_Medical_LevelAdd=M("Medical_level");       
        ...
        if($_POST['id']){
             if($vifnn_Medical_LevelAdd->where(array('lid'=>(int)$_POST['id']))->save($data)){
                $this->success('修改成功',U('Medical/Level',array('token'=>$data['token'])));exit;
             }else{
                $this->error('修改失败');
             }
        }else{
            if($vifnn_Medical_LevelAdd->add($data)){
