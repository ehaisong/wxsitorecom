...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'vifnnif'        =>  array('attr'=>'where','level'=>2),
		...
		'vifnnlist'    =>  array('attr'=>'array,foreach,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	...
	public function _vifnnif($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'vifnnif');
	...
	public function _vifnnlist($attr,$content) {
        static $_iterateParseCache = array();
        //如果已经解析过，则直接返回变量值
        $cacheIterateId = md5($attr.$content);
        if(isset($_iterateParseCache[$cacheIterateId]))
            return $_iterateParseCache[$cacheIterateId];
        $tag      = $this->parseXmlAttr($attr,'vifnnlist');
        $name   = $tag['array'];
        $id        = $tag['foreach'];
        $empty  = isset($tag['empty'])?$tag['empty']:'';
        $key     =   !empty($tag['key'])?$tag['key']:'i';
